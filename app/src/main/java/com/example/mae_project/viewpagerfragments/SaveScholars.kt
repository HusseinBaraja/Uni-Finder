...
class SaveScholars : BaseFragment(R.layout.fragment_university) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvHeader.text = "Saved ScholarShips"
        binding.btnLogOut.beGone()

        dataBaseViewModel.loadScholars()
        initRecycler(adaptorInit = {
            ...
            scholarAdaptor?.onItemCLick = {
                ...
                InfoBottomFragment().show(childFragmentManager.beginTransaction(), "Save Scholar")
                Unit
            }
        }, recycler = {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenCreated {
            dataBaseViewModel.scholarData.collectLatest {
                when (it) {
                    is DataStates.Empty -> {
                        with(binding) {
                            mEmpty.beVisible()
